...
import { Ticket, Fuel, Check, TrendingUp, TrendingDown, DollarSign } from 'lucide-react';
...
const ZERO_LOTTERY = { scratch_sales: '', scratch_payouts: '', lotto_sales: '', lotto_payouts: '', books_activated: '', books_settled: '', notes: '' };
const ZERO_FUEL = { regular_gallons: '', regular_price: '', plus_gallons: '', plus_price: '', premium_gallons: '', premium_price: '', diesel_gallons: '', diesel_price: '', notes: '' };
...
export default function LotteryFuelPage() {
  ...
  const [lForm, setLForm] = useState(ZERO_LOTTERY);
  const [fForm, setFForm] = useState(ZERO_FUEL);
  ...
  const saveLottery = async (e: React.FormEvent) => {
    ...
    setLForm(ZERO_LOTTERY); fetchData();
  };
      ...
      <div className="space-y-5">
        ...
        <div className="flex gap-2">
          <button onClick={() => setTab('lottery')}
            className={cn('flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-all', tab === 'lottery' ? 'bg-fire-700 text-white shadow-fire-sm' : 'border border-dragon-border text-obsidian-400 hover:border-fire-800 hover:text-fire-400')}>
            <Ticket className="h-4 w-4" />Lottery
          </button>
          <button onClick={() => setTab('fuel')}
            className={cn('flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold transition-all', tab === 'fuel' ? 'bg-fire-700 text-white shadow-fire-sm' : 'border border-dragon-border text-obsidian-400 hover:border-fire-800 hover:text-fire-400')}>
            ...
            <div className="d-card p-5">
              ...
              <form onSubmit={saveLottery} className="space-y-4">
                <div>
                  ...
                  <input type="date" value={lDate} onChange={e => setLDate(e.target.value)} className="d-input w-48" />
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="d-label text-obsidian-400">🎟 Scratch-Off Sales</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-obsidian-500">$</span>
                      <input type="number" step="0.01" min="0" value={lForm.scratch_sales} onChange={e => lf('scratch_sales', e.target.value)} className="d-input pl-7" placeholder="0.00" /></div>
                  </div>
                  <div>
                    <label className="d-label text-fire-400">🎟 Scratch-Off Payouts</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-obsidian-500">$</span>
                      <input type="number" step="0.01" min="0" value={lForm.scratch_payouts} onChange={e => lf('scratch_payouts', e.target.value)} className="d-input pl-7" placeholder="0.00" /></div>
                  </div>
                  <div>
                    <label className="d-label text-obsidian-400">🎰 Lotto Terminal Sales</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-obsidian-500">$</span>
                      <input type="number" step="0.01" min="0" value={lForm.lotto_sales} onChange={e => lf('lotto_sales', e.target.value)} className="d-input pl-7" placeholder="0.00" /></div>
                  </div>
                  <div>
                    <label className="d-label text-fire-400">🎰 Lotto Payouts</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-obsidian-500">$</span>
                      <input type="number" step="0.01" min="0" value={lForm.lotto_payouts} onChange={e => lf('lotto_payouts', e.target.value)} className="d-input pl-7" placeholder="0.00" /></div>
                  </div>
                  <div>
                    <label className="d-label">Books Activated</label>
                    <input type="number" min="0" value={lForm.books_activated} onChange={e => lf('books_activated', e.target.value)} className="d-input" placeholder="0" />
                  </div>
                  <div>
                    <label className="d-label">Books Settled</label>
                    <input type="number" min="0" value={lForm.books_settled} onChange={e => lf('books_settled', e.target.value)} className="d-input" placeholder="0" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="d-label">Notes</label>
                    <input value={lForm.notes} onChange={e => lf('notes', e.target.value)} className="d-input" placeholder="Optional notes" />
                  </div>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-obsidian-900/50">
                    <tr>
                      {['Date', 'Scratch Sales', 'Scratch Paid', 'Lotto Sales', 'Lotto Paid', 'Books Act.', 'Books Set.', 'NET'].map(h => (
                        <th key={h} className="px-3 py-2.5 text-left text-xs font-semibold text-obsidian-500">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-dragon-border">
                    {lotteryEntries.map(e => (
                      <tr key={e.id} className="hover:bg-obsidian-900/20">
                        ...
                        <td className={cn('mono px-3 py-2.5 font-bold', Number(e.total_net) >= 0 ? 'text-gold-400' : 'text-fire-400')}>
                          {fmt.currency(e.total_net)}
                        </td>
                      </tr>
                    ))}
                    {!loading && lotteryEntries.length === 0 && (
                      <tr><td colSpan={8} className="px-3 py-8 text-center text-obsidian-500">No entries yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              <div className="d-card p-4">
                <p className="text-xs text-obsidian-500 mb-1">Total Gallons (30d)</p>
                <p className="mono text-xl font-bold text-white">{fmt.number(fuelTotals.gallons, 1)} gal</p>
              </div>
              <div className="d-card p-4 border-fire-900/50">
                <p className="text-xs text-obsidian-500 mb-1">Fuel Revenue (30d)</p>
                <p className="mono text-xl font-bold text-fire-400">{fmt.currency(fuelTotals.sales)}</p>
              </div>
            </div>
            ...
            <div className="d-card p-5">
              ...
              <form onSubmit={saveFuel} className="space-y-4">
                <div>
                  <label className="d-label">Date</label>
                  <input type="date" value={fDate} onChange={e => setFDate(e.target.value)} className="d-input w-48" />
                </div>
                  ...
                  {[
                    ...
                    <div key={fuel.grade} className="d-card p-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="d-label">Gallons sold</label>
                          <input type="number" step="0.01" min="0" value={(fForm as any)[fuel.gKey]} onChange={e => ff(fuel.gKey, e.target.value)} className="d-input" placeholder="0.00" />
                        </div>
                        <div>
                          <label className="d-label">Price/gallon $</label>
                          <input type="number" step="0.001" min="0" value={(fForm as any)[fuel.pKey]} onChange={e => ff(fuel.pKey, e.target.value)} className="d-input" placeholder="0.000" />
                        </div>
                      </div>
                      {num((fForm as any)[fuel.gKey]) > 0 && (
                        <p className="mt-1.5 text-xs text-fire-400 mono">= {fmt.currency(num((fForm as any)[fuel.gKey]) * num((fForm as any)[fuel.pKey]))}</p>
                      )}
                    </div>
                  ))}
                </div>
                <div>
                  <label className="d-label">Notes</label>
                  <input value={fForm.notes} onChange={e => ff('notes', e.target.value)} className="d-input" placeholder="Optional" />
                </div>
                <button type="submit" disabled={saving} className="btn-fire">
            ...
            <div className="d-card overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-obsidian-900/50">
                    <tr>
                      {['Date', 'Regular', 'Plus', 'Premium', 'Diesel', 'Total Gal', 'Total Sales'].map(h => (
                        <th key={h} className="px-3 py-2.5 text-left text-xs font-semibold text-obsidian-500">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-dragon-border">
                    ...
                    {!loading && fuelEntries.length === 0 && (
                      <tr><td colSpan={7} className="px-3 py-8 text-center text-obsidian-500">No fuel entries yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
