...
import { Calculator, TrendingUp, DollarSign, ShoppingCart, Plus, Minus, RefreshCw } from 'lucide-react';
...
export default function ReportsPage() {
  ...
  const [scanProductHistory, setScanProductHistory] = useState<any>(null);

  const [adjustments, setAdjustments] = useState([
    { id:'scratch_off', label:'Scratch-Off Lottery Sold', amount:0, type:'deduct' as 'deduct'|'add' },
  ...
  const fetchData = useCallback(async () => {
    ...
    setSales(salesData??[]); setSaleItems(items); setLoading(false);
  }, [store, period]);

  useEffect(() => { fetchData(); }, [fetchData]);

  const handleScan = async (result: {barcode:string;product:any}) => {
    setScanResult(result);
    if (result.product && store) {
      // Pull this product's sales history for the current period
      const sb = createClient();
      const now = new Date();
      const from = period==='today'?startOfDay(now):period==='week'?startOfWeek(now):period==='month'?startOfMonth(now):subDays(now,30);
      const { data: items } = await sb.from('sale_items')
        .select('quantity,unit_price,unit_cost,line_total,created_at')
        .eq('product_id', result.product.id)
        .gte('created_at', from.toISOString());
      const totalQty = (items??[]).reduce((s,i)=>s+Number(i.quantity),0);
      const totalRev = (items??[]).reduce((s,i)=>s+Number(i.line_total),0);
      const totalProfit = (items??[]).reduce((s,i)=>s+(Number(i.unit_price)-Number(i.unit_cost))*Number(i.quantity),0);
      setScanProductHistory({ name: result.product.name, qty: totalQty, revenue: totalRev, profit: totalProfit, transactions: items?.length ?? 0 });
    }
  };
  ...
  for (const li of saleItems) {
    ...
    vendorMap.set(key,{revenue:cur.revenue+Number(li.line_total),profit:cur.profit+(Number(li.unit_price)-Number(li.unit_cost))*Number(li.quantity),qty:cur.qty+Number(li.quantity)});
  }
  ...
  for (let i=days-1;i>=0;i--) dayMap.set(format(subDays(new Date(),i),period==='today'?'ha':'MMM d'),0);
  for (const s of sales) { const key=format(new Date(s.created_at),period==='today'?'ha':'MMM d'); dayMap.set(key,(dayMap.get(key)??0)+Number(s.total)); }
  ...
  const Tip = ({active,payload,label}:any) => active&&payload?.length?(<div className="d-card p-2 text-xs"><p className="text-obsidian-400 mb-1">{label}</p><p className="text-fire-400">{fmt.currency(payload[0].value)}</p></div>):null;

  return (
    <AppShell title="Sales Reports" storeName={store?.name}>
      <div className="space-y-5">
        {scanResult && (
          <ScanToast barcode={scanResult.barcode} product={scanResult.product} onClose={()=>{setScanResult(null);setScanProductHistory(null);}} />
        )}

        {/* Scanner + period picker */}
        <div className="flex flex-wrap items-center gap-3 justify-between">
          ...
          {store && (
            <BarcodeScanner storeId={store.id} onScan={handleScan} placeholder="Scan to see product sales…" className="w-64" />
          )}
        </div>

        {/* Scanned product sales history */}
        {scanProductHistory && (
          <div className="d-card p-4 border-fire-900/40 bg-fire-950/20">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-white">{scanProductHistory.name}</p>
                <p className="text-xs text-obsidian-400 mt-0.5">Sales in this period</p>
              </div>
              <button onClick={()=>setScanProductHistory(null)} className="text-obsidian-600 hover:text-obsidian-300 text-xs">clear</button>
            </div>
            <div className="grid grid-cols-4 gap-3 mt-3">
              {[
                {label:'Units Sold',value:String(scanProductHistory.qty)},
                {label:'Revenue',value:fmt.currency(scanProductHistory.revenue)},
                {label:'Profit',value:fmt.currency(scanProductHistory.profit)},
                {label:'Transactions',value:String(scanProductHistory.transactions)},
              ].map(k=>(
                <div key={k.label} className="text-center">
                  <p className="mono text-lg font-bold text-fire-400">{k.value}</p>
        ...
        <div className="d-card p-5 border-fire-900/40 shadow-fire-sm">
          ...
          <div className="space-y-3">
            ...
            {customAdj.map(adj=>(
              <div key={adj.id} className={cn('flex items-center gap-3 rounded-lg px-3 py-2',adj.type==='deduct'?'bg-fire-950/30':'bg-obsidian-800/30')}>
                <div className={cn('flex h-5 w-5 items-center justify-center rounded text-white text-xs font-bold shrink-0',adj.type==='deduct'?'bg-fire-700':'bg-obsidian-600')}>{adj.type==='deduct'?'−':'+'}</div>
                <span className="text-sm text-obsidian-300 flex-1">{adj.label}</span>
                <span className={cn('mono text-sm font-medium',adj.type==='deduct'?'text-fire-400':'text-obsidian-300')}>{adj.type==='deduct'?'−':'+' }{fmt.currency(adj.amount)}</span>
                <button onClick={()=>setCustomAdj(a=>a.filter(x=>x.id!==adj.id))} className="text-obsidian-600 hover:text-fire-500"><Minus className="h-3.5 w-3.5"/></button>
              </div>
            ))}
            <div className="flex gap-2 pt-2">
              <input value={newAdjLabel} onChange={e=>setNewAdjLabel(e.target.value)} placeholder="Label (e.g. Cash over/short)" className="d-input flex-1 h-8 text-xs" />
            ...
            <div className="mt-3 flex items-center justify-between rounded-xl bg-fire-900/30 border border-fire-800/50 px-4 py-3">
              ...
              <span className="mono text-xl font-bold text-fire-gradient">{fmt.currency(netSales)}</span>
            </div>
          </div>
        </div>
            ...
            {vendorMap.size===0?<p className="text-sm text-obsidian-500">No sales yet.</p>:(
              <div className="space-y-2.5">
                {[...vendorMap.entries()].sort((a,b)=>b[1].revenue-a[1].revenue).map(([name,v])=>{
                  const vc=VENDOR_COMPANIES.find(x=>x.name===name);
                  return(
                    <div key={name} className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-base">{vc?.emoji??'⚪'}</span>
                        <div><p className="text-sm font-medium text-white">{name}</p><p className="text-xs text-obsidian-500">{v.qty} units · profit {fmt.currency(v.profit)}</p></div>
                      </div>
        ...
        <div className="d-card overflow-hidden">
          <div className="border-b border-dragon-border px-5 py-3.5"><h3 className="font-semibold text-white">Best Selling Products</h3></div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-obsidian-900/50">
                <tr>{['#','Product','Units','Revenue','Profit','Margin'].map(h=><th key={h} className={cn('px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-obsidian-500',['Units','Revenue','Profit','Margin'].includes(h)?'text-right':'')}>{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-dragon-border">
                ...
                {topProducts.map((p,i)=>(
                  <tr key={p.name} className="hover:bg-obsidian-900/30">
