...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!apiKey) return NextResponse.json({ error: 'OPENROUTER_API_KEY not set in environment variables' }, { status: 500 });

    const formData = await request.formData();
    const files: File[] = [...(formData.getAll('file') as File[]).filter(f => f.size > 0)];
    ...
    const content: any[] = [{
      type: 'text',
      text: 'Extract this invoice data. Return ONLY valid JSON, no markdown:\n{"vendor_name":null,"vendor_company":null,"invoice_number":null,"invoice_date":null,"total_amount":null,"line_items":[{"description":"name","quantity":1,"unit_cost":0.00}]}'
    }];

    for (const file of files) {
      const buf = Buffer.from(await file.arrayBuffer());
      if (file.type === 'application/pdf') {
        content.push({ type: 'file', file: { filename: file.name, file_data: `data:application/pdf;base64,${buf.toString('base64')}` } });
      } else {
        content.push({ type: 'image_url', image_url: { url: `data:${file.type || 'image/jpeg'};base64,${buf.toString('base64')}` } });
      }
      try { await createAdminClient().storage.from('invoices').upload(`${store.id}/${crypto.randomUUID()}.jpg`, buf, { contentType: file.type }); } catch {}
    }
    ...
    let parsed: any;
    try { parsed = JSON.parse(text.replace(/^```json\s*/i,'').replace(/^```\s*/i,'').replace(/```\s*$/i,'').trim()); }
    catch { return NextResponse.json({ error: 'AI response not parseable — try a clearer photo' }, { status: 502 }); }
    ...
    const lineItems = (parsed.line_items ?? []).map((item: any) => {
      ...
      return {
        raw_description: item.description ?? 'Unknown', quantity: Number(item.quantity) || 0, unit_cost: cost,
        old_cost: oldCost, line_total: (Number(item.quantity) || 0) * cost,
        ...
        is_new_product: !match, current_price: match?.unit_price ?? null, action: match ? 'update' : 'create',
      };
    });
    ...
    const { data: invoice, error: invErr } = await admin.from('invoices').insert({
      ...
      price_changes_count: lineItems.filter((li: any) => li.price_changed).length,
      raw_ai_response: parsed,
    }).select('id').single();

    if (invErr) return NextResponse.json({ error: `DB error: ${invErr.message} — run schema.sql in Supabase` }, { status: 500 });
    ...
    return NextResponse.json({
      invoice: { id: invoice?.id, vendor_name: parsed.vendor_name, vendor_company: parsed.vendor_company, invoice_number: parsed.invoice_number, total_amount: parsed.total_amount, status: 'NEEDS_REVIEW', price_changes_count: lineItems.filter((li: any) => li.price_changed).length },
