...
export async function POST(request:NextRequest){
  try{
    ...
    const{data:products}=await admin.from('products').select('id,name,sku,unit_cost,unit_price,quantity,min_quantity,max_quantity,vendor_company').eq('store_id',store.id).eq('is_active',true).eq('vendor_company',vendorName);
    ...
    const{data:salesData}=await admin.from('sale_items').select('product_id,quantity,line_total').in('product_id',ids).gte('created_at',ago);
    ...
    const{data:order}=await admin.from('vendor_orders').insert({store_id:store.id,vendor_id:vendorId??null,vendor_name:vendorName,status:'draft',ai_generated:!!apiKey,ai_reasoning:reasoning,total_estimated:total}).select('id').single();
    if(order&&orderItems!.length)await admin.from('vendor_order_items').insert(orderItems!.map((i:any)=>({order_id:order.id,product_id:i.product_id,product_name:i.product_name,sku:i.sku??null,current_stock:i.current_stock,suggested_qty:i.suggested_qty,unit_cost:i.unit_cost,line_total:i.line_total,reason:i.reason})));
