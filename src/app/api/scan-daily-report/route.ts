...
report_type: store_close, till, lottery, scratch, department, safe_drop, paid_out, paid_in, fuel, register, summary, unknown
Use null for missing fields. Never guess.`
  ...
  catch { throw new Error(`Invalid JSON: ${match[0].slice(0,100)}`); }
}
...
function merge(existing: any, u: any): any {
  ...
  if (t === 'store_close' || t === 'summary') {
    return { ...existing,
      ...
      net_sales: n(u.net_sales)||n(existing.net_sales),
      fuel_sales: keep('fuel_sales'), inside_sales: keep('inside_sales'),
      merchandise_sales: keep('merchandise_sales'), taxes: keep('taxes'),
      discounts: keep('discounts'), refunds: keep('refunds'),
      transactions: keep('transactions'), customers: keep('customers'),
      credit_sales: keep('credit_sales'), debit_sales: keep('debit_sales'),
      ebt_sales: keep('ebt_sales'), check_sales: keep('check_sales'),
    };
  }
  if (t === 'till' || t === 'register') {
    return { ...existing,
      cash_sales: sum('cash_sales'), safe_drops: sum('safe_drops'),
      paid_outs: sum('paid_outs'), paid_ins: sum('paid_ins'),
      actual_cash: sum('actual_cash'), expected_cash: sum('expected_cash'),
      drawer_difference: sum('drawer_difference'),
      short_over: sum('short_over'), check_sales: sum('check_sales'),
    };
  }
  ...
  // Unknown — merge all non-null non-zero
  const m = { ...existing };
...
function validate(r: any): string[] {
  ...
  if (act > 0 && exp > 0 && Math.abs(act - exp) > 50) {
  ...
  if (gross > 0 && pmts > 0 && Math.abs(gross - pmts) > gross * 0.15) {
    w.push(`Payment methods ($${pmts.toFixed(2)}) don't match gross sales ($${gross.toFixed(2)})`);
  }
  return w;
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: upload } = await sb.from('report_uploads').insert({
      ...
      file_name: files.map((f: File) => f.name).join(', '), status: 'completed',
      raw_extraction: extracted, parsed_data: extracted,
      ai_notes: extracted.notes || null, processed_at: new Date().toISOString(),
    ...
    // Calculate short/over: actual - expected
    ...
    if (upload && savedReport) {
      await sb.from('report_uploads').update({ daily_report_id: savedReport.id }).eq('id', (upload as any).id).catch(() => {});
    }
