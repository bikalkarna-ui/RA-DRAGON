...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (reportId) {
      await sb.from('report_uploads').delete().eq('daily_report_id', reportId);
      await sb.from('register_reports').delete().eq('daily_report_id', reportId);
      ...
      if (dr) {
        await sb.from('report_uploads').delete().eq('daily_report_id', dr.id);
        await sb.from('register_reports').delete().eq('daily_report_id', dr.id);
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const { id, store_notes, status } = body;
